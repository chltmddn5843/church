...
export default async function LegacyContentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!Number.isInteger(legacyId) || legacyId < 11 || legacyId > 223) notFound()
  const page = await getDb().select().from(contentPages).where(and(eq(contentPages.legacyId, legacyId), eq(contentPages.published, true))).get()
  if (!page) notFound()
