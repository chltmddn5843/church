  ...
  return (
    <>
      <div className="border-b border-[#dedede] pb-7">
        ...
        <p className="mt-2 text-sm text-[#777]">홈페이지에 표시할 이미지를 등록하고 노출 상태를 관리합니다.</p>
      </div>

      <PopupForm />

      <section className="mt-14">
        <div className="mb-4 flex items-end justify-between"><h2 className="text-lg font-bold">등록된 팝업</h2><span className="text-xs text-[#999]">총 {items.length}개</span></div>
        {items.length === 0 ? (
          ...
          <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {items.map((item) => (
              <article key={item.id} className="overflow-hidden border border-[#dedede] bg-white">
                ...
                <div className="p-4"><h3 className="truncate font-semibold">{item.title}</h3>{item.content && <p className="mt-1 line-clamp-2 text-sm text-[#777]">{item.content}</p>}
                  <div className="mt-4 flex gap-2 border-t pt-4">
                    <form action={togglePopup.bind(null, item.id, !item.active)} className="flex-1"><Button size="sm" variant="outline" className="w-full">{item.active ? <EyeOff /> : <Eye />}{item.active ? "숨기기" : "노출하기"}</Button></form>
                    <form action={deletePopup.bind(null, item.id)}><Button size="sm" variant="destructive" aria-label={`${item.title} 삭제`}><Trash2 /></Button></form>
                  </div>
                </div>
              </article>
            ))}
