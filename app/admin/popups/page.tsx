  ...
  return (
    <>
      <div className="border-b border-[#dedede] pb-7">
        <p className="text-sm font-medium text-[#2F5D8A]">POPUP MANAGEMENT</p>
            ...
            {items.map((item) => (
              <article key={item.id} className="overflow-hidden border border-[#dedede] bg-white">
                <div className="relative aspect-[4/3] bg-[#f3f3f3]">
                  ...
                  <span className={`absolute left-3 top-3 px-2 py-1 text-xs font-medium ${item.active ? "bg-[#123A63] text-white" : "bg-[#526a7d] text-white"}`}>{item.active ? "노출 중" : "숨김"}</span>
                </div>
                <div className="p-4">
