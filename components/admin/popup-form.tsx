  ...
  return (
    <form action={createPopup} className="mt-8 border-t-2 border-[#333] pt-7">
      <div className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(220px,1fr)]">
        <label className="grid gap-2 text-sm font-medium">제목
        ...
        <label className="grid gap-2 text-sm font-medium">연결 주소 <span className="font-normal text-[#999]">(선택)</span>
      ...
      <div className="mt-7 flex items-center justify-between">
        <p className="text-sm font-medium">팝업 이미지</p>
        <label className="flex items-center gap-2 text-sm"><input type="checkbox" name="active" defaultChecked className="size-4" /> 바로 활성화</label>
      </div>
      ...
      <label className="relative mt-2 flex min-h-[340px] cursor-pointer items-center justify-center overflow-hidden border border-dashed border-[#bbb] bg-[#fafafa] transition hover:bg-[#f5f5f5]">
        ...
        {preview ? (
          ...
          <span className="flex flex-col items-center text-center text-[#777]">
            ...
            <strong className="text-sm text-[#333]">이미지를 선택하거나 이곳에 놓아주세요</strong>
            <span className="mt-2 text-xs">JPG, PNG, WEBP, GIF · 최대 5MB</span>
          </span>
        )}
      </label>

      {fileName && <div className="flex items-center justify-between border border-t-0 bg-[#fafafa] px-4 py-3 text-sm"><span className="truncate">{fileName}</span><button type="button" aria-label="선택 취소" onClick={() => selectFile()}><X className="size-4" /></button></div>}

      <label className="mt-7 grid gap-2 text-sm font-medium">팝업 설명 <span className="font-normal text-[#999]">(선택)</span>
