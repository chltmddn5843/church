      ...
      <div className="mx-auto grid max-w-6xl gap-4 px-4 sm:grid-cols-2 lg:grid-cols-4">
        {links.map((link) => (
          <Link
            key={link.title}
            href={link.href}
            className="group flex min-h-64 flex-col items-center justify-center rounded-[2rem] border border-white/70 bg-white/55 px-7 py-8 text-center text-foreground shadow-lg shadow-primary/10 backdrop-blur-sm transition-all hover:-translate-y-1 hover:bg-white/75 hover:shadow-xl active:scale-[0.98] active:bg-white/35"
          >
