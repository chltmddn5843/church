import type { SVGProps } from "react"
import { MessageCircle } from "lucide-react"
import { cn } from "@/lib/utils"

function InstagramIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" {...props}>
      <rect x="2" y="2" width="20" height="20" rx="5" ry="5" />
      <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
      <line x1="17.5" y1="6.5" x2="17.51" y2="6.5" />
    </svg>
  )
}

function YoutubeIcon(props: SVGProps<SVGSVGElement>) {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" {...props}>
      <path d="M22.54 6.42a2.78 2.78 0 0 0-1.94-2C18.88 4 12 4 12 4s-6.88 0-8.6.46a2.78 2.78 0 0 0-1.94 2A29 29 0 0 0 1 11.75a29 29 0 0 0 .46 5.33A2.78 2.78 0 0 0 3.4 19c1.72.46 8.6.46 8.6.46s6.88 0 8.6-.46a2.78 2.78 0 0 0 1.94-2 29 29 0 0 0 .46-5.25 29 29 0 0 0-.46-5.33z" />
      <polygon points="9.75 15.02 15.5 11.75 9.75 8.48 9.75 15.02" />
    </svg>
  )
}

const links = [
  {
    label: "원당교회 카카오톡 채널",
    href: "https://pf.kakao.com/_gxjxkxnX",
    Icon: MessageCircle,
    className: "bg-[#FEE500] text-[#3C1E1E]",
  },
  {
    label: "원당교회 인스타그램",
    href: "https://www.instagram.com/wondang1964/",
    Icon: InstagramIcon,
    className: "bg-gradient-to-tr from-[#f9ce34] via-[#ee2a7b] to-[#6228d7] text-white",
  },
  {
    label: "원당교회 유튜브",
    href: "https://www.youtube.com/@wondang1964",
    Icon: YoutubeIcon,
    className: "bg-[#FF0000] text-white",
  },
] as const

export function SocialFloat() {
  return (
    <div className="fixed bottom-6 right-4 z-40 flex flex-col gap-3 md:right-6">
      {links.map(({ label, href, Icon, className }) => (
        <a
          key={label}
          href={href}
          target="_blank"
          rel="noopener noreferrer"
          aria-label={label}
          className={cn(
            "flex size-11 items-center justify-center rounded-full shadow-lg transition hover:scale-105 focus-visible:scale-105",
            className,
          )}
        >
          <Icon className="size-5" />
        </a>
      ))}
    </div>
  )
}
